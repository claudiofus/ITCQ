...
class QuizFragment : Fragment()
{
	                          ...
	                          savedInstanceState : Bundle?) : View?
	{
		...
		refreshQuestion(mQuestionView, answerArr, randomInt)
		...
		//				if (ans != null && ans.isChecked) {
		//					activity.database.use {
		//						val parserVerify = rowParser { text : String -> Answer(text) }
		//						val resultVerify = select(Answer.TABLE_NAME).column(
	...
	private fun refreshQuestion(mQuestionView : TextView?, answerArr : Array<ToggleButton?>,
	                            randomInt : String)
	{
		activity.database.use {
			val parserQue = rowParser { id : Int, text : String -> Question(id, text) }
			val parserAns = rowParser { text : String -> Answer(text) }
			...
			val resultAns = select(Answer.TABLE_NAME).column(Answer.COLUMN_TEXT).whereSimple(
